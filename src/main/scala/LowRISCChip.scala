...
class OuterMemorySystem extends Module with TopLevelParameters {
  ...
  if(params(NBanks) > 1) {
    ...
    tagCache.io.uncached <> arb.io.out
  } else {
    //conv.io.uncached <> masterEndpoints.head.io.outer
    tagCache.io.uncached <> masterEndpoints.head.io.outer
   }
  ...
  if(params(UseBackupMemoryPort)) {
    ...
    VLSIUtils.doOuterMemorySystemSerdes(tagCache.io.mem, io.mem, io.mem_backup,
                                        io.mem_backup_en, htifW)
  } else {
    //io.mem <> conv.io.mem 
    io.mem <> tagCache.io.mem 
  }
}
